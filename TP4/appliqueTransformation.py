...
def appliqueTransformation(img, H):
    ...
    upperLeft = [0, 0, 1]
    upperRight = [cols - 1, 0, 1]
    lowerLeft = [0, rows - 1, 1]
    lowerRight = [cols - 1, rows - 1, 1]
    inv_h = inv(H)
    ...
    corners = np.array((newUpperLeft, newUpperRight, newLowerRight, newLowerLeft))
    ...
    new_image = np.zeros((ceil(new_size[1]), ceil(new_size[0]), 3), dtype='uint8')
